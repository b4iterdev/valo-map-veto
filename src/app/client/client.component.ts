...
export class ClientComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.targetId) {
        this.sessionService.getSession(this.targetId);
      }
    });
  ...
  getCurrentTurn(session: Session): string {
    ...
    this.currentVeto = session.vetoOrder?.find(
      (veto) => veto.order === this.curOrder,
    );
    if (!this.currentVeto) {
    ...
    const team =
      this.currentVeto.map === 0 ? session.leftTeam : session.rightTeam;
    ...
    if (this.currentVeto.type === 'ban') {
      ...
      if (this.currentVeto.side === undefined) {
        ...
        const sidePicker =
          this.currentVeto.side === 0 ? session.leftTeam : session.rightTeam;
  ...
  onMapClick(map: MapState, session: Session) {
    ...
    // Handle decider action
      ...
      const remainingMap = session.mapStates.find(
        (m) => !m.banned && !m.picked,
      );
      if (remainingMap) {
  ...
  onSideSelect(side: number) {
    ...
    if (
      this.currentVeto.type === 'ban' ||
      this.currentVeto.side === undefined
    ) {
      console.log('Invalid side selection state');
      return;
    }
    // Find the map in the session's mapStates array
    const mapIndex = this.curSession.mapStates.findIndex(
      (m) => m.name === this.map?.name,
    );
    if (mapIndex === -1) {
    ...
    if (this.targetId && this.curSession.id) {
      this.sessionService.updateMapState(
        this.targetId,
        this.curSession.mapStates,
      );
    }
  }
  showSide(map: MapState): string {
    if (
      ...
      !map.order ||
      map.side === undefined ||
      map.selectedBy === undefined
    ) {
      return '';
    }

    // Find the corresponding veto order for this map
    const matchingVeto = this.curSession.vetoOrder?.find(
      (veto) => veto.order === map.order,
    );
